...
public class FileSizeUtil {
    ...
    private static double FormatFileSize(long size, int sizeType) {
        ...
        switch (sizeType) {
            case SIZE_TYPE_B:
                fileSizeLong = ConverterUtil.getDouble(df.format((double) size));
                break;
            case SIZE_TYPE_KB:
                fileSizeLong = ConverterUtil.getDouble(df.format((double) size / 1024));
                break;
            case SIZE_TYPE_MB:
                fileSizeLong = ConverterUtil.getDouble(df.format((double) size / 1048576));
                break;
            case SIZE_TYPE_GB:
                fileSizeLong = ConverterUtil.getDouble(df.format((double) size / 1073741824));
                break;
            default:
                break;
        }

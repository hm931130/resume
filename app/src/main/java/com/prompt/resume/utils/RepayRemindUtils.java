...
public class RepayRemindUtils {
    ...
    //如果没有存在的账户就往系统里面添加一个
    private static String CALENDARS_NAME = "dami";
    ...
    public static boolean insertCalendarEvent(Context context, String title, String description,
                                              long beginTimeMillis, long endTimeMillis, String location) {
        ...
        if (endTimeMillis == 0) {
//            endTimeMillis = beginTimeMillis + 24 * 60 * 60 * 1000;
            endTimeMillis = beginTimeMillis;
            beginTimeMillis = endTimeMillis - 24 * 60 * 60 * 1000;
        }
...
//            reminderValues.put(CalendarContract.Reminders.MINUTES, 10); // 提前10分钟提醒
     ...
     * @return
     */
    public static boolean isExistCalendarEvent(Context context, String title) {
        if (context == null) {
            return false;
        }
        ...
        try {
            if (eventCursor != null) {
                for (eventCursor.moveToFirst(); !eventCursor.isAfterLast(); eventCursor.moveToNext()) {
                    ...
                    if (!TextUtils.isEmpty(title) && title.equals(eventTitle)) { //如果存在日历事件
                        return true;
                    }
                }
            }

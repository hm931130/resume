...
public class CityListAdapter extends RecyclerView.Adapter<CityListAdapter.BaseViewHolder> {
    ...
    private static final int VIEW_TYPE_RECENT = 12;

    private Context mContext;
    private List<City> mData;
    private List<HotCity> mHotData;
    private List<RecentCity> mRecentData;
    ...
    public CityListAdapter(Context context, List<City> data, List<HotCity> hotData, List<RecentCity> recentData, int state) {
        this.mData = data;
        this.mContext = context;
        this.mHotData = hotData;
        this.mRecentData = recentData;
    ...
    public void onBindViewHolder(@NonNull BaseViewHolder holder, int position) {
        ...
        if (holder instanceof LocationViewHolder) {
            ...
            int itemWidth = (screenWidth - padding - space * (GridListAdapter.SPAN_COUNT - 1) - indexBarWidth) /3;
        ...
        if (holder instanceof RecentViewHolder) {
            ...
            final City data = mData.get(pos);
            if (data == null) return;
            GridRecentAdapter mAdapter = new GridRecentAdapter(mContext, mRecentData);
            mAdapter.setInnerListener(mInnerListener);
            ((RecentViewHolder) holder).mRecyclerView.setAdapter(mAdapter);
        }
    }
    ...
    public int getItemViewType(int position) {
        ...
        if (position == 1 && TextUtils.equals("最", mData.get(position).getSection().substring(0, 1)))
    ...
    public static class RecentViewHolder extends BaseViewHolder {
        RecyclerView mRecyclerView;
